...
export default function ProtectedPreview({ html, watermarkText = 'Criador de Logomarca', isPaid = false }: ProtectedPreviewProps) {
  return (
    ...
    >
      ...
      {!isPaid && (
        <div 
          className="absolute inset-0 pointer-events-none select-none overflow-hidden opacity-15 flex flex-wrap justify-center items-center gap-x-6 gap-y-6 p-4"
          style={{ transform: 'rotate(-25deg) scale(2)' }}
        >
          {Array.from({ length: 64 }).map((_, i) => (
            <span key={i} className="text-gray-400 font-black text-xl select-none whitespace-nowrap tracking-tighter">
              {watermarkText}
            </span>
          ))}
        </div>
